...
class MyNode(Node):
    ...
    def send_mode_call(self):
        if self.gyro_flag == True and self.tof1_flag == True and self.tof2_flag == True and self.tof3_flag == True and self.flag_belok_capit == False:
            ...
            # info.data = servo_calculation(int(5)) #ganti flag depannya
            # info.data[0] = int(6)
            ...
            if self.mode1 == True:
                if self.tof1 <= 20 and self.tof2 <= 30 and self.flag == 10:
                    ...
                    #nyapit
                    info = Int32MultiArray
                    if self.cam[1] == "disebelah kanan":
                        info.data = servo_calculation(3) #ganti flag depannya
                        info.data[0] = 5
                        self.publish_data.publish(info)
                    elif self.cam[1] == "disebelah kiri":
                        info.data = servo_calculation(-3) #ganti flag depannya
                        info.data[0] = 6
                        self.publish_data.publish(info)
                    else:
                        if self.cam[0] > 20:
                            maju_mundur = Int32()
                            maju_mundur.data = 3 #maju/mundur berapa jauh sama yaw || yaw, jarak, angle
                            self.maju_mundur_data.publish(maju_mundur)
                            self.get_logger().info("Maju")
                        else:
                            data = Int32
                            data.data = int(self.cam[0])
                ...
                elif self.tof1 > 20 or self.kiri == False:
                    if abs((self.tof2 + self.tof3)/2) > 15 and self.kiri == True and self.flag == 11:
                        belok = Int32()
                        belok.data = 22
                        self.belok_data.publish(belok)
                        self.flag_belok_capit = True

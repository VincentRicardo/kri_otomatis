...
class MyNode(Node):
    def __init__(self):
        ...
        self.subscriber_data = self.create_subscription(Int32, "/capit", self.fungsi_, 1)
    ...
    def fungsi_(self, message = Int32):
      if self.capit_flag == False:
          ...
          info.data = servo_calculation(int(message.data)) #nurunin capit + maju
          message.data[0] = 2
          self.publish_data.publish(info)
          tahan_waktu(3)
          info.data = [4, 65, 90, 140, 85, 105 ,25, 40, 110, 180, 30, 80, 5, 55, 85, 150, 80, 85, 15] #nyapit
          self.publish_data.publish(info)
          tahan_waktu(3)

...
class MyNode(Node):                      
    ...
    def ngitung_gyro(self):
        ...
        GyroX = GyroX - 0.005335400000000001 # cek imu error
        GyroY = GyroY - 0.11128120000000012 # cek imu error
        GyroZ = GyroZ + 0.06352936999999995 # cek imu error
        AccelX = data[6]
        AccelY = data[7]
        AccelZ = data[8]
        AccelAngleY = ((math.atan(-1 * (AccelX)/math.sqrt(math.pow((AccelY),2) + math.pow((AccelZ),2)))*180/math.pi)) - 0.2852022588717437 # cek imu error

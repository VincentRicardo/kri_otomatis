...
def calculate_IMU_error(): 
    ...
    c = 0
    error = [GyroErrorX, GyroErrorY, GyroErrorZ, AccelErrorX, AccelErrorY]
    return error
...
class MyNode(Node):                      
    def __init__(self):
        ...
        self.subscriber_flag_belok_capit = self.create_subscription(String, "/flag_belok_capit", self.flag, 1)
        ...
        self.gyroAngleX = 0
        self.gyroAngleY = 0
        #self.error = calculate_IMU_error()
   
    def flag(self, message:String):
        self.yaw = 0
        self.pitch = 0
    ...
    def ngitung_gyro(self):
        ...
        GyroX = GyroX + 3.5985496183206105#- self.error[0]  # cek imu error
        GyroY = GyroY - 2.7114885496183216#self.error[1] # cek imu error
        GyroZ = GyroZ + 0.15484732824427483 #self.error[2] # cek imu error
        ...
        AccelAngleY = ((math.atan(-1 * (AccelX)/math.sqrt(math.pow((AccelY),2) + math.pow((AccelZ),2)))*180/math.pi)) + 0.9663632744202275#- self.error[4] # cek imu error
        ...
        self.pitch = (0.96 * self.gyroAngleY + 0.04 * AccelAngleY)

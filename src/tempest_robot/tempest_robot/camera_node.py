...
class Image(Node):
    ...
    def camera_read(self):
        ...
        results = self.model(frame, conf=0.85)[0]   
        #line1 = cv2.line(frame, (250,0), (250,640), (255,0,0))
        #line1 = cv2.line(frame, (400,0), (400,640), (255,0,0))
        ...
        for r in results:
            ...
            for box in boxes :
                ...
                #if obj_center_x <320:
                    #posisi = "kiri"
                #elif obj_center_x >350:
                    #posisi = "kanan"
                #else :
                    #posisi = "tengah"

                self.distance_center = (((self.frame_width/2)- obj_center_x)/10)
                camera_width = x2 - x1
                self.distance = (knownWidth * focalLength)/ camera_width

                #self.angle = math.degrees(math.atan2(self.center_x - ((x1+x2)/2 ), self.distance))

                self.distance = round(self.distance,2)
                self.angle = round(self.angle, 2)
                self.distance_center =round(self.distance_center)
                ...
                #cv2.putText(frame_gray, "D_C: {:.2f} cm".format(self.distance_center),(x1, y1-25) , cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 2)
    ...
    def kirim_(self, message: String):
        ...
        message.data = [int(self.distance), int(self.distance_center)]
